...
contract("SingleDynamicPullPayment", (accounts) => {
  ...
  describe("subscribeToBillingModel()", async () => {
    ...
    it("should subscribe the billing correctly", async () => {
      ...
      expect(subscritionId[subscritionId.length - 1]).to.bignumber.be.eq(
        currentSubscriptionId
      );

      const subscription = await this.contract.getSubscription(
        currentSubscriptionId
      );
      const currentPullPaymentId =
        await this.contract.getCurrentPullPaymentId();
