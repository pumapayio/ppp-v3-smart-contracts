...
require('chai').should();
const { addLiquidity, swap, createPair } = require('./helpers/swap');
...
contract.skip('Executor', (accounts) => {
	...
	describe('Swap with PMA (CAKE)', () => {
		beforeEach(async () => {
			...
			await contracts.registry.contract.setAddressFor('PMAToken', contracts.cakeToken.address, {
				from: owner
			});
		});
	...
	describe('Swap with PMA', () => {
		it('should subscribe billing model with WBNB when ADA is settlement token ', async () => {
			await createPair(this.factory, WBNB_ADDRESS, pmaToken.address);
			...
			await createPair(this.factory, ADA_ADDRESS, pmaToken.address);
		...
		it('should subscribe billing model with PMA when WBNB is settlement token ', async () => {
			...
			const tx = await this.contract.subscribeToBillingModel(1, pmaToken.address, {
				from: customer
			});
